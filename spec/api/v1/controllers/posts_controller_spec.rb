...
RSpec.describe Api::V1::PostsController, type: :controller do
  ...
  context "unauthenticated user" do
    it "PUT update returns http unauthenticated" do
      put :update, topic_id: my_topic.id, id: my_post.id, post: {title: "Post title", body: "Here is the body of my post"}
      expect(response).to have_http_status(401)
    end

    it "POST create returns http unauthenticated" do
      post :create, topic_id: my_topic.id, post: {title: "Post title", body: "Here is the body of my post"}
      expect(response).to have_http_status(401)
    end
  ...
  context "authorized and authenticated users" do
    before do
      my_user.admin!
      controller.request.env['HTTP_AUTHORIZATION'] = ActionController::HttpAuthentication::Token.encode_credentials(my_user.auth_token)
      @new_post = build(:post)
