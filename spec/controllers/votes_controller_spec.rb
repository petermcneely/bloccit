...
RSpec.describe VotesController, type: :controller do
  ...
  context "signed-in user" do
    ...
    describe "POST up_vote" do
      ...
      it "the users second vote does not increase the number of post votes" do
        post :up_vote, format: :js, post_id: user_post.id
        votes = user_post.votes.count
        post :up_vote, format: :js, post_id: user_post.id
      ...
      it "increases the sum of votes by one" do
        points = user_post.points
        post :up_vote, format: :js, post_id: user_post.id
        expect(user_post.points).to eq(points + 1)
      end

      it "returns http success for posts show page" do
        request.env["HTTP_REFERER"] = topic_post_path(my_topic, user_post)
        post :up_vote, format: :js, post_id: user_post.id
        expect(response).to have_http_status(:success)
      end

      it "returns http success for posts topic show page" do
    ...
    describe "POST down_vote" do
      ...
      it "the users second vote does not decrease the number of post votes" do
        post :down_vote, format: :js, post_id: user_post.id
        votes = user_post.votes.count
        post :down_vote, format: :js, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes)
      ...
      it "decreases the sum of votes by one" do
        points = user_post.points
        post :down_vote, format: :js, post_id: user_post.id
        expect(user_post.points).to eq(points - 1)
      end
